...
示例:
  ...
  python run_tests.py test_cases.json --junit-xml report.xml --workers 4
    ...
    parser.add_argument(
        ...
        nargs="?",
        default=None,
        help="测试配置文件路径（.json 或 .yaml），默认使用脚本所在目录下的 test_cases.json",
    )
    ...
    if config_file is None:
        config_file = os.path.join(workspace, "test_cases.json")
    ...
    if args.junit_xml:
        ...
        write_junit_xml(
            runner.results,
            junit_path,
            suite_name=os.path.splitext(os.path.basename(config_file))[0],
        )
